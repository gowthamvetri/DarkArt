  ...
  try {
    ...
    const payload = list_items.map((item) => {
      return {
        ...
        paymentId: "",
        orderQuantity: item.quantity, // Use individual item quantity
        orderDate: new Date(),
        paymentStatus: "CASH ON DELIVERY",
        deliveryAddress: addressId,
        subTotalAmt: subTotalAmt,
        totalAmt: totalAmount,
        orderStatus: "ORDER PLACED" // Add initial status
      };
    });
    ...
    try {
        ...
        try {
            ...
            await ProductModel.findByIdAndUpdate(
                order.productId,
                { 
                    $inc: { stock: order.orderQuantity } // Add back the cancelled quantity
                },
                { session, new: true }
            );
                        ...
                        html: `
                          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 5px;">
                            ...
                            <div style="background-color: #fff3cd; padding: 15px; border-radius: 5px; margin: 20px 0; border-left: 4px solid #ffc107;">
                              ...
                              <p><strong>Product:</strong> ${order.productDetails?.name}</p>
                              <p><strong>Quantity:</strong> ${order.orderQuantity}</p>
                              <p><strong>Cancellation Date:</strong> ${new Date().toLocaleDateString()}</p>
                              <p><strong>Stock Restored:</strong> Yes, ${order.orderQuantity} units have been added back to inventory</p>
                            </div>
    ...
    try {
        ...
        try {
            ...
            const updatedOrder = await orderModel.findOneAndUpdate(
             ...
             .populate("productId", "name image price stock") // Include stock in population
                        ...
                        html: `
                          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 5px;">
                            ...
                            <div style="background-color: #f5f5f5; padding: 15px; border-radius: 5px; margin: 20px 0; border-left: 4px solid #4caf50;">
                              ...
                              ${orderStatus === "CANCELLED" ? `<p style="color: #d32f2f;"><strong>Stock Restored:</strong> ${order.orderQuantity} units have been added back to inventory.</p>` : ''}
                            </div>
