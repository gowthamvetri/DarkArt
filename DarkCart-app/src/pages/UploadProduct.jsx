    ...
    <div className='grid p-3'>
      <form className='grid gap-2'>
        ...
        <div>
          <p>Image</p>
        <div>
        <label htmlFor='ProductImage' className='bg-blue-100 h-24 p-2 outline-none flex justify-center border focus-within:border-blue-300 rounded cursor-pointer'>
            <div className=' text-center flex flex-col items-center justify-center'>
              {
                imageLoading ? <Loading/> : (
                    <>
                      <FaCloudUploadAlt className='text-4xl text-blue-400'size={35} />
                      <p>Upload Image</p></>
                )
              }
...
<div className='my-2 '>
{
  data.image.map((img,index) =>{
    return(
      <div key ={img+index} className='w-24 h-24 min-w-20 bg-blue-50 border'> 
      <img src={img} alt={`Product Image ${index + 1}`} className='w-full h-full object-scale-down' />
      </div>
    )
  })
